...
The references do not need to be perfect design assets. They just need to make the intended hierarchy, spacing, and direction concrete enough that Codex is not guessing.

## Be specific

The more specific you are about the expected interaction patterns and the style you want, the better the result will be.
The model tends to default to high-frequency patterns and style so if it's not clear from your references that you want something else, the UI might look generic.
...
If necessary, such as when you're not using a standard stack, specify to Codex which primitives to reuse, where your tokens live, and what the repo considers canonical for buttons, inputs, cards, typography, and icons.

If you're starting from an existing codebase, Codex will likely understand on its own how to use your components and design system, but if starting from scratch, it's a good idea to be explicit.
...
Make sure you have the Playwright interactive skill enabled in Codex. For more details, see the [skills documentation](https://developers.openai.com/docs/build-skills).
...
Use additional screenshots or short notes if they help clarify states that are not clear from one image.
