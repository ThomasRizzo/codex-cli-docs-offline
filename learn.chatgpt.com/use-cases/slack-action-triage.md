...
summary: Use ChatGPT with Slack and the tools where work happens to find direct
...
skills:
  ...
  - token: google-calendar
    ...
    description: Check event timing when a thread depends on a meeting, launch,
      webinar, or deadline.
bestFor:
  - People who get work through Slack and need ChatGPT to separate live asks
    from already-handled chatter.
...
After trying this and tweaking the flow to match your needs, you can [schedule a task for the work from the chat](https://developers.openai.com/codex/automations#schedule-a-task-inside-a-chat) by asking ChatGPT to do the same thing on a schedule.
