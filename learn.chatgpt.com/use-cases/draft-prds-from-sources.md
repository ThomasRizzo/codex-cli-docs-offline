...
summary: Use ChatGPT with the $documents skill and connected plugins such as
  Linear, Slack, Notion or Google Drive to create a reviewable PRD with the
  expected sections, a timeline, decisions, open questions, and a source
  appendix.
skills:
  - token: $documents
...
Start with the sources you want ChatGPT to use: the Linear project, the Slack planning channel or thread, and any Drive docs, Notion pages, meeting notes, or local files that should be cited in the PRD.
...
2. Name the sources directly: the Linear project or milestone, the Slack channel or thread, and the docs or notes ChatGPT should cite.
3. Give ChatGPT the PRD section contract.
4. Review the source appendix first, then the requirements and open questions.
5. Use the same chat to resolve gaps, tighten scope, and prepare the handoff.
...
## Refine in the same chat

Use the starter prompt on this page for the first draft. If something is missing, point ChatGPT at the missing source instead of starting over.

## Check the source trail

Before sharing the PRD, ask ChatGPT to list the claims with weak or missing support, the unresolved questions, and the decisions it treated as confirmed. If the source appendix does not make those easy to audit, keep refining the same chat before exporting or posting anything.
