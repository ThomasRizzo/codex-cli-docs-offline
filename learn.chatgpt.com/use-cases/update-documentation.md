...
For a repo-wide convention, add documentation expectations to [AGENTS.md](https://developers.openai.com/codex/agent-configuration/agents-md). For example:
...
You can also [schedule a task for this workflow from the current chat](https://developers.openai.com/codex/automations#schedule-a-task-inside-a-chat). For example, ask Codex to fetch recent GitHub pull requests and keep the docs current every week:
...
```text
Schedule a task for the workflow above from this chat. Fetch all the recent PRs in [this repo/linked repo] and update docs based on the changes.
```