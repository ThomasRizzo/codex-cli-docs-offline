...
tagline: Review single-cell QC, annotations, and UMAPs in one chat.
summary: Use ChatGPT with the NGS Analysis plugin to turn a 10x-style matrix
  bundle into QC-filtered single-cell artifacts, threshold-justified filtering
  summaries, annotations, and UMAPs you can inspect and revise in the same chat.
skills:
  - token: NGS Analysis
...
1. Point ChatGPT to the appropriate matrix, barcodes, genes or features, manifest, and dataset metadata, or provide exact file references.
2. Run the starter prompt so ChatGPT can choose QC thresholds from the observed distributions and record the rationale in the run artifacts.
...
4. Continue in the same chat to refine thresholds, supply a matched reference atlas, or rerun after unblocking doublet detection.
...
Finally, review the cell-level metrics and filtering outcomes. ChatGPT preserves
this table with the filtered `.h5ad` and visualization artifacts so you can
revise the thresholds in the same chat without losing the rationale for the
first pass.
