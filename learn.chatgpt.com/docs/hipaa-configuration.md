...
OpenAI provides resources to help you configure Codex, including [our web page](https://openai.com/index/running-codex-safely/), [documentation](configuration.html), and the [Codex Security Whitepaper](https://trust.openai.com/?itemUid=382f924d-54f3-43a8-a9df-c39e6c959958&source=click) on the Trust Portal. Review this material before allowing Codex to be used with PHI. This guide provides examples of how some of these tools can be used.
...
The following sections explain how you can use the `requirements.toml` policy configuration file and related settings to control how Codex operates. Review the [OpenAI documentation](configuration.html) for other settings, and revisit that review periodically as Codex capabilities change.

## Enable Codex

Follow the [Codex Enterprise admin setup instructions](enterprise/admin-setup.html) to enable Codex Local for your workspace and confirm that users have the required permissions and that you have a BAA with OpenAI.
...
| ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [Sign-in method](auth.html)                                                                                    | ChatGPT sign-in; API key sign-in only with BAA-covered API Services with Modified Retention.                   | Determines whether ChatGPT workspace controls or API organization controls apply. |
| [Approval policy](agent-approvals-security.html#automatic-approval-reviews)                                    | `allowed_approval_policies`: `"on-request"`, `"untrusted"`, `"never"`, and inline-table `granular`<sup>1</sup> | Configures when Codex requests approval.                                          |
| [Approval reviewer](enterprise/managed-configuration.html#configure-automatic-review-policy)                   | `allowed_approvals_reviewers = ["user", "auto_review"]`                                                        | Configures how Codex routes sandbox boundary approvals.                           |
| [Permission profiles](enterprise/managed-configuration.html#control-available-permission-profiles)<sup>2</sup> | `default_permissions = ":workspace"`<br />Allow only `:read-only` and `:workspace`.                            | This policy allows read-only and workspace access, but not full access.           |
| [Web search](config-file/config-basic.html#web-search-mode)                                                    | `allowed_web_search_modes = ["cached", "indexed", "live", "disabled"]`                                         | Configures how Codex uses the web.                                                |
| [Browser and computer-use features](enterprise/managed-configuration.html#pin-feature-flags)                   | `true` or `false`                                                                                              | Configures surface-specific features.                                             |
| [MCP servers](extend/mcp.html)                                                                                        | Leave `[mcp_servers]` empty by default; allowlist only exact, approved servers.                                | Disables local MCP services by default. Add only approved servers or connectors.  |
...
1. **Select the approved sign-in path.** Decide whether users will [authenticate to Codex Local](auth.html) with ChatGPT, use API keys, or use both for separate workflows.
...
3. **Enable Codex Local and define RBAC groups.** Use [Codex Enterprise admin setup](enterprise/admin-setup.html) to enable Codex Local, create a small Codex Admin group, and assign Codex access through RBAC groups such as Codex Users and Codex PHI Users.
...
5. **Train users on approvals and sandbox boundaries.** Use [Agent approvals and security](agent-approvals-security.html) to explain when Codex can act inside the sandbox, when it asks for approval, and why users should review network, file-transfer, repository-write, and third-party connector actions.
...
- [Codex authentication](auth.html)
- [Codex config basics](config-file/config-basic.html)
- [Codex Enterprise admin setup](enterprise/admin-setup.html)
- [Codex managed configuration](enterprise/managed-configuration.html)
- [Agent approvals and security](agent-approvals-security.html)
- [Codex Model Context Protocol](extend/mcp.html)
- [Codex governance](enterprise/governance.html)
- [Codex permissions](permissions.html)
