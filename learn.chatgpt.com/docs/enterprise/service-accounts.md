...
A service account represents a non-human workspace identity. A [personal access token](access-tokens.html) represents the workspace member who creates it. API Platform project service accounts and API keys use separate project access and billing.
...
For more about workspace permissions, see [Roles and workspace permissions](roles-and-workspace-permissions.html).
...
Assign the identity to the workspace and required groups, then sync it. The identity provider manages the account's name, group membership, and lifecycle. SCIM-managed accounts can't be renamed or deleted in ChatGPT. See [Groups and provisioning](groups-and-provisioning.html).
...
- [Personal access tokens](access-tokens.html)
...
- [Compliance API and audit events](compliance-api.html)
