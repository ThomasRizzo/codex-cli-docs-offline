...
3. Choose the replacement identity. For a durable non-human workflow on an
   ...
   account](service-accounts.html). Otherwise, identify an
...
[Access tokens](access-tokens.html#rotate-or-revoke-a-token).
...
[Roles and workspace permissions](roles-and-workspace-permissions.html)
for the complete boundary model and [Plugin controls](apps-and-connectors.html)
...
overview, see [Compliance API and audit events](compliance-api.html).
...
- [Service accounts](service-accounts.html)
