...
[Configure plugin marketplaces and defaults](managed-configuration.html#configure-plugin-marketplaces-and-defaults).
...
Use **Installation policy** to choose **Available** or **Installed** for each eligible role. Required apps must also be enabled, and members must have access to the connected service. Importing a plugin does not grant app access or connect members' accounts. See [Plugin controls](apps-and-connectors.html) for role, app, and action controls.
