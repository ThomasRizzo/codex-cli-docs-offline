...
1.  Select "Worktree"
    ...
    Optionally, choose a [local environment](local-environment.html) to run setup scripts for the worktree.
...
1. [Work exclusively on the worktree](#option-1-working-on-the-worktree). This path works best when you can verify changes directly on the worktree, for example because you have dependencies and tools installed using a [local environment setup script](local-environment.html).
