...
The `@openai/codex-security` package is public. Running scans requires Codex
  Security access. Follow the [CLI quickstart](../cli.html) to install
  the CLI and sign in.
...
setup](reference.html#use-openrouter-or-fireworks) or [Amazon
Bedrock setup](reference.html#use-amazon-bedrock).
...
reference](reference.html#codex-security-bulk-scan). For common
