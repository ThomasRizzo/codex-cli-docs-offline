...
with the [CLI quickstart](../cli.html).
...
see the [CLI reference](reference.html#select-scan-authentication).
...
See [Run bulk security scans](bulk-scans.html) for GitHub
...
context](reference.html#add-security-context).
...
artifacts](reference.html#scan-artifacts) for the full layout.
...
reference](reference.html#codex-security-findings).
...
configuration](reference.html#configure-deep-scans).
...
selected severity. See [Run scans in CI](ci.html) for the
complete GitHub Actions workflow, artifact handling, and SARIF export.

### Can another application run scans directly

Yes. Use the [TypeScript SDK](../sdk.html) to start scans, select
