...
Backlog triage starts from existing findings. To search the repository for new
  vulnerabilities, [run a security scan](scans.html). Triage
...
- the [`$codex-security:fix-finding`](fix-findings.html)
...
- `confirmed`: After a person accepts the finding for remediation, use
  [`$codex-security:fix-finding`](fix-findings.html) to fix and
