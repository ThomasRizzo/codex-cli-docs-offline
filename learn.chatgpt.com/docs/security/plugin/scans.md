...
scan](deep-scans.html). Deep scans take longer and search
more extensively.
...
- **Code changes:** [Review code
  changes](code-changes.html) when a pull request or commit
...
- **After a fix:** [Fix and verify the
  finding](fix-findings.html). Confirm that the issue no
...
findings](export-findings.html).

## Next step

After you accept a finding, use [Fix and verify a
finding](fix-findings.html) to generate and review one
