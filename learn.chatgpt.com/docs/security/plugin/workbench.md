...
Install and enable the [Codex Security plugin](../plugin.html), then
...
See [Run a security scan](scans.html), [Run a deep security
scan](deep-scans.html), or [Review code changes for
security](code-changes.html) for details about each scan
type.
...
findings](fix-findings.html) for the remediation workflow.
...
The **Findings** tab shows findings from saved Codex Security scans. Imported
  ...
  [backlog triage workflow](triage-backlog.html).
