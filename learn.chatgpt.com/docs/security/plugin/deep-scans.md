...
Start with a [standard scan](scans.html) to check your scope
...
[plugin changelog](changelog.html) for release details.
...
changelog](changelog.html) before you update the plugin or
...
finding](fix-findings.html).

To review a pull request, commit, branch range, or local patch, use [Review code
changes](code-changes.html). A deep scan never substitutes
