...
The `@openai/codex-security` package is public. Running scans requires Codex
  ...
  Security plugin quickstart](plugin.html). For connected GitHub
  repositories, see [Codex Security cloud setup](setup.html).
...
prerequisites](cli/reference.html#authentication-and-prerequisites).
...
See also [CLI reference](cli/reference.html).
...
setup](cli/reference.html#use-amazon-bedrock). For [OpenRouter or
Fireworks](cli/reference.html#use-openrouter-or-fireworks), set the
...
permissions](cli/reference.html#local-scan-permissions).
...
The [CLI reference](cli/reference.html#scan-artifacts) describes
...
[`validate` and `patch` reference](cli/reference.html#codex-security-validate-and-codex-security-patch).
...
[Run bulk security scans](cli/bulk-scans.html).
...
the [CLI reference](cli/reference.html).

Continue with the workflow that fits your goal:

- [Run bulk security scans](cli/bulk-scans.html) to discover GitHub
  repositories or scan a pinned CSV inventory.
- [Read the CLI FAQ](cli/faq.html) for answers about scan history,
  false-positive feedback, coverage, and fix verification.
- [Run scans in CI](cli/ci.html) to review pull requests, preserve
  results, and set a severity policy.
- [Use the CLI reference](cli/reference.html) to check every flag,
  output format, artifact, and exit code.
- [Integrate the TypeScript SDK](sdk.html) to run scans from an
