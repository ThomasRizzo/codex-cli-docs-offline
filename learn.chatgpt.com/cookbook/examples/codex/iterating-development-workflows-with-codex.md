...
For new projects, the best place to start is by populating the [`AGENTS.md`](../../../docs/agent-configuration/agents-md.html) file. When present, Codex reads this file as persistent repository guidance before performing work. It is an optional convention for work that benefits from durable execution. In general, agents files should contain items like:
...
Given the size of each prompt and the number of files involved, it should seem obvious that such a manual process would become unwieldy, error-prone and unpredictable. To make such work reusable and more consistent, Codex provides [skills](../../../docs/build-skills.html) as a way of creating workflows that can be reused for projects of any scale.
