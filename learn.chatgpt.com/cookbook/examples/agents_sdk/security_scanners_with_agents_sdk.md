...
Both examples use static source analysis. They don't invoke [Codex Security](../../../docs/security.html), run either application, or modify the source. You could build on this workflow by adding runtime checks or applying fixes, with human approval before either step.
...
We pick the completed review with the most confirmed findings and turn those findings into a [`/goal` prompt for Codex](../../../use-cases/follow-goals.html). Python does the formatting to avoid additional model calls. In under 4,000 characters, the prompt tells Codex which verified files to work on and asks for focused regression tests.
